...
"""
Модуль для поддержки принятия решений и анализа согласованности экспертных оценок.
Содержит функции для работы с матрицами парных сравнений и вычисления весов альтернатив.
"""
...
def normalize_matrix(matrix):
    ...
    Каждый элемент делится на сумму элементов своего столбца.

    Args:
        matrix (np.array): Исходная матрица парных сравнений
...
def calculate_weights_eigenvector(matrix):
    """
    Вычисление весов методом собственного вектора (метод Аналитической Иерархии Процессов).

    Args:
        matrix (np.array): Матрица парных сравнений

    Returns:
        tuple: (веса альтернатив, собственное значение, индекс согласованности CI, отношение согласованности CR)
    """
...
def calculate_weights_log_least_squares(matrix):
    """
    Вычисление весов методом логарифмических наименьших квадратов.

    Args:
        matrix (np.array): Матрица парных сравнений

    Returns:
        np.array: Веса альтернатив
    """
    n = matrix.shape[0]
...
def calculate_weights_geometric_mean(matrix):
    """
    Вычисление весов методом среднего геометрического.
...
def calculate_weights_line_method(matrix):
    """
    Вычисление весов методом "линия" (Line Method).
...
def calculate_weights_ahp(matrix):
    """
    Вычисление весов с использованием метода анализа иерархий (AHP) с дополнительными проверками.
    ...
    if CR > 0.1:
        print("Внимание: Отношение согласованности CR > 0.1. Матрица может быть несогласованной.")

    return weights, lambda_max, CI, CR

def analyze_consistency(matrix):
    """
    Полный анализ согласованности матрицы парных сравнений.

    Args:
        matrix (np.array): Матрица парных сравнений

    Returns:
        dict: Результаты анализа согласованности
    """
    try:
...
def generate_consistent_matrix(weights):
    """
    Генерация согласованной матрицы парных сравнений на основе весов.

    Args:
        weights (list or np.array): Веса альтернатив

    Returns:
        np.array: Согласованная матрица парных сравнений
    """
    n = len(weights)
...
def check_transitivity(matrix):
    """
    Проверка транзитивности матрицы парных сравнений.

    Args:
        matrix (np.array): Матрица парных сравнений

    Returns:
        tuple: (флаг транзитивности, список несогласованных троек)
    """
    n = matrix.shape[0]
...
def create_sensitivity_analysis(matrix, results, perturbation_factor=0.1):
    """
    Анализ чувствительности к изменениям в матрице.

    Args:
        matrix (np.array): Исходная матрица
        results (dict): Результаты анализа
        perturbation_factor (float): Фактор возмущения

    Returns:
        dict: Результаты анализа чувствительности
    """
    n = matrix.shape[0]
    sensitivity_results = {}
    try:
...
def adjust_inconsistent_pairs(matrix, inconsistent_pairs, threshold=0.1):
    """
    Автоматическая корректировка несогласованных парных сравнений.

    Args:
        matrix (np.array): Исходная матрица
        inconsistent_pairs (list): Список несогласованных пар
        threshold (float): Порог несогласованности

    Returns:
        tuple: (скорректированная матрица, список измененных пар)
    """
...
def export_to_latex_table(df, caption="Таблица", label="tab:example"):
    """
    Экспорт DataFrame в LaTeX таблицу.

    Args:
        df (pd.DataFrame): DataFrame для экспорта
        caption (str): Подпись таблицы
        label (str): Метка таблицы

    Returns:
        str: LaTeX код таблицы
    """
...
def save_session(session_data, filename):
    """
    Сохранение сессии анализа в файл.
...
def load_session(filename):
    """
    Загрузка сессии анализа из файла.

    Args:
        filename (str): Имя файла для загрузки

    Returns:
        dict: Данные сессии или None при ошибке
    """
    try:
