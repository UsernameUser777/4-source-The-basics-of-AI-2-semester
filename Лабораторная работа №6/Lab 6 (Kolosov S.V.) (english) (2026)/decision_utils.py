...
# Decision-making utilities: risk matrix, profile, Venn, export, recommendations
...
def calculate_risk_matrix(
    payoffs: List[List[float]],  # [alternatives][states]
    probabilities: List[float]
) -> Tuple[List[List[float]], float]:
    """
    Calculate the risk matrix (regret) and expected risk for each alternative.
    """
    try:
        payoffs_np = np.array(payoffs, dtype=np.float64)
        if payoffs_np.ndim != 2:
            raise ValueError("payoffs must be a 2D matrix")

        n_alternatives, n_states = payoffs_np.shape
        if len(probabilities) != n_states:
            logger.error(f"Length of probabilities {len(probabilities)} does not match number of states {n_states}")
            return [], 0.0

        # Maximum payoff per state
        ...
        # Expected risk for each alternative
        ...
        logger.error(f"Error in calculate_risk_matrix: {e}")
        return [], 0.0
...
def plot_profile_risk_return(
    ...
    """
    Plot risk-return profile for alternatives.
    """
    try:
        if not alternatives:
            logger.error("plot_profile_risk_return: alternatives list is empty")
            return None, None
        ...
        if not returns or not risks:
            logger.error("plot_profile_risk_return: empty return or risk data")
            return None, None
        ...
        logger.error(f"Error in plot_profile_risk_return: {e}")
        return None, None
...
def plot_venn_diagram(
    ...
    """
    Plot a Venn diagram for 2 or 3 sets.
    """
    try:
        try:
        ...
        if len(sets) == 2 and has_venn:
            try:
                ...
                venn2(
                    ...
                    set_labels=labels or ("Set A", "Set B"),
                    ax=ax
                )
                ax.set_title(title)
                plt.tight_layout()
...
def export_results_to_excel(data: Dict[str, Any], filepath: str) -> bool:
    ...
    """
    try:
        # Import functions from risk_analysis for calculations
        try:
            from risk_analysis import calculate_expected_value, calculate_variance_and_std, calculate_utility
        except ImportError:
            logger.warning("Failed to import functions from risk_analysis")
        ...
        if 'options' in data:
            ...
            for opt in data['options']:
                row = {
                    ...
                    'Expected Value': calculate_expected_value(
                        opt.get('outcomes', []),
                        opt.get('probabilities', [])
                    ),
                    'Standard Deviation': calculate_variance_and_std(
                        opt.get('outcomes', []),
                        opt.get('probabilities', []),
                        0
                    )[1],
                ...
                rows.append(row)
            df_data['Options Analysis'] = pd.DataFrame(rows)

        if 'monte_carlo' in data:
            mc_data = data['monte_carlo']
            df_data['Monte Carlo'] = pd.DataFrame({
                'Metric': ['Mean', 'Std Dev', 'Min', 'Max', 'CI_Lower', 'CI_Upper'],
                'Value': [
        ...
        if not df_data:
            logger.warning("export_results_to_excel: no data to export")
            return False
        ...
        logger.error(f"Error exporting to Excel {filepath}: {e}")
        return False

class PDFReport(FPDF):
    def header(self):
        self.set_font('Arial', 'B', 15)
        self.cell(0, 10, 'Lab Work №6. Risk Analysis', 0, 1, 'C')
    def footer(self):
        self.set_y(-15)
...
def export_results_to_pdf(data: Dict[str, Any], filepath: str) -> bool:
    ...
    try:
        ...
        if 'summary' in data:
            ...
            pdf.cell(0, 10, "No text report to export.", ln=True)

        pdf.output(filepath)
        logger.info(f"Results exported to PDF: {filepath}")
        return True
    except Exception as e:
        logger.error(f"Error exporting to PDF {filepath}: {e}")
        return False

def fuzzy_set_membership(x: float, params: Dict[str, float]) -> float:
    """
    Calculate membership degree to a fuzzy set (triangular).
    """
    try:
        a = params.get("a", 0.0)
        b = params.get("b", 1.0)
...
def generate_recommendations(data: Dict[str, Any]) -> str:
    ...
    try:
        ...
        if 'monte_carlo' in data:
            ...
            if std_dev > 20:
                recommendations += "- Diversification or hedging is recommended due to high risk (Std.Dev > 20).\n"
            else:
                recommendations += "- Risk level (Std.Dev) is acceptable.\n"
        ...
        if 'var' in data:
            ...
            if var_val < -50:
                recommendations += "- VaR indicates potential significant losses (>50).\n"

        if 'risk_matrix' in data:
            _, min_exp_risk = data['risk_matrix']
            if min_exp_risk > 10:
                recommendations += "- Minimum expected risk is above the threshold.\n"
...
if __name__ == "__main__":
    print("--- Testing decision_utils.py ---")
    ...
    print(f"Risk matrix:\n{risk_mat}")
    print(f"Min. expected risk: {min_risk}")
    ...
    if fig_v:
        ...
        print("Venn diagram saved to venn_test.png")

    data_for_export = {
        "options": [
            {"name": "Option 1", "outcomes": [100, -50], "probabilities": [0.6, 0.4]},
            {"name": "Option 2", "outcomes": [80, 20], "probabilities": [0.5, 0.5]}
        ],
        "monte_carlo": {
            "mean": 100.5, "std_dev": 15.2, "min": 70.0, "max": 130.0,
            "ci_lower": 85.0, "ci_upper": 115.0
        },
        "summary": "This is an example text report."
    }
    ...
    print("File decision_utils.py is working correctly.")
