...
# Полная реализация визуализации с обработкой всех ошибок и соответствием ТЗ

import matplotlib

matplotlib.use('TkAgg')  # Важно для работы с Tkinter в main.py
...
# Установим стиль по умолчанию (темный режим)
sns.set_style("darkgrid")


def safe_save_fig(fig: Optional[plt.Figure], filepath: str) -> bool:
    """
    Безопасное сохранение графика — проверяем fig != None
    """
    if fig is None:
        logger.error(f"Не удалось сохранить график: fig == None (путь: {filepath})")
        return False
    try:
        ...
        logger.error(f"Ошибка при сохранении графика {filepath}: {e}")
        return False
...
def plot_decision_tree(
    ...
    """
    Построение дерева решений с узлами решений и исходов.
    """
    if not options:
        logger.error("plot_decision_tree: список опций пуст")
        return None, None
    ...
    try:
        ...
        for i, option in enumerate(options):
            ...
            if len(outcomes) != len(probabilities):
                logger.warning(f"Несоответствие длин outcomes и probabilities в опции '{option.get('name', '?')}'")
            ...
            node_text = f"{option.get('name', 'Опция')}\nEV: {ev:.2f}\nU: {utility:.2f}"
            ...
            for j, (outcome, prob) in enumerate(zip(outcomes, probabilities)):
                ...
                outcome_text = f"Исход: {outcome:.2f}\nP: {prob:.2f}"
        ...
        logger.error(f"Ошибка в plot_decision_tree: {e}")
        return None, None


def plot_probability_distribution(
        options: List[Dict[str, Any]],
        title: str = "Распределение вероятностей"
) -> Tuple[Optional[plt.Figure], Optional[plt.Axes]]:
    """
    Построение распределения вероятностей для опций.
    """
    if not options:
        logger.error("plot_probability_distribution: список опций пуст")
        return None, None
    ...
    try:
        ...
        for i, opt in enumerate(options):
            ...
            if outcomes.size == 0 or probs.size == 0:
                logger.warning(f"Пропущена опция '{opt.get('name', '?')}' — пустые outcomes/probabilities")
                continue

            if outcomes.size != probs.size:
                logger.warning(f"Пропущена опция '{opt.get('name', '?')}' — несоответствие размеров")
                continue

            total_prob = np.sum(probs)
            if not np.isclose(total_prob, 1.0, rtol=1e-3):
                probs = probs / (total_prob + 1e-12)
                logger.info(f"Нормализованы вероятности для '{opt.get('name', '?')}'")
            ...
            ax.stem(outcomes, probs, linefmt=f'{colors[i]}-', markerfmt=f'{colors[i]}o',
                    basefmt="", label=f'{opt.get("name", f"Опция {i + 1}")} (EV={ev:.2f})')
            ax.axvline(ev, color=colors[i], linestyle='--', linewidth=1.2)

            has_valid_data = True

        if not has_valid_data:
            logger.error("plot_probability_distribution: нет корректных данных для построения графика")
            plt.close(fig)
            return None, None
        ...
        logger.error(f"Ошибка в plot_probability_distribution: {e}")
        import traceback
...
def plot_weibull_analysis(
        shape: float = 1.0,
        scale: float = 1.0,
        title: str = "Анализ распределения Вейбулла"
) -> Tuple[Optional[plt.Figure], Optional[plt.Axes]]:
    """
    Построение PDF и CDF распределения Вейбулла.
    """
    try:
        if shape <= 0 or scale <= 0:
            logger.error(f"Некорректные параметры Вейбулла: shape={shape}, scale={scale}")
            return None, None
        ...
        logger.error(f"Ошибка в plot_weibull_analysis: {e}")
        return None, None
...
def plot_expected_shortfall(
    ...
    """
    График ES для нормального распределения.
    """
    try:
        if not (0 < confidence_level < 1):
            logger.error(f"Некорректный уровень доверия: {confidence_level}")
            return None, None
        ...
        logger.error(f"Ошибка в plot_expected_shortfall: {e}")
        return None, None
...
def plot_risk_attitude(
    ...
    """
    График функции полезности и вторая производная → отношение к риску.
    """
    try:
        risk_params = risk_params or {}
        ...
        ax[0].plot(x, u, 'b-', label='Полезность')
        ax[0].set_ylabel("U(x)")
        ax[0].legend()
        ax[0].grid(True, linestyle='--', alpha=0.6)
        ...
        if avg_sec < 0:
            attitude = "Рискофобия (неприятие риска)"
        elif avg_sec > 0:
            attitude = "Рискофилия (склонность к риску)"
        else:
            attitude = "Нейтральное отношение к риску"

        fig.suptitle(f"Отношение к риску: {attitude}", fontsize=14)
        plt.tight_layout()
        return fig, ax
    except Exception as e:
        logger.error(f"Ошибка в plot_risk_attitude: {e}")
        return None, None
...
def plot_venn_diagram(
    ...
    """
    Упрощённая диаграмма Венна для 2–3 множеств.
    """
    try:
        has_venn = False
        try:
        ...
        if len(sets) == 2 and has_venn:
            ...
            venn2(subsets=(len(sets[0] - sets[1]), len(sets[1] - sets[0]), len(sets[0] & sets[1])),
                  set_labels=labels or ("Множество А", "Множество В"), ax=ax)
            ax.set_title(title)
            return fig, ax
